...
// Only keep first 45 posts, then append the new deterministic ones
const trimmedData = data.slice(0, 45); 

POSTS_DEF.forEach(def => {
  const title = formatTitle(def.slug);
  const primaryProduct = PRODUCTS[def.primary] || PRODUCTS['AS3'];
  const secondaryProduct = def.secondary ? PRODUCTS[def.secondary] : PRODUCTS['AS3'];
